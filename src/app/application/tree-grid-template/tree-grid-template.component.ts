...
export class TreeGridTemplateComponent implements OnInit, OnDestroy,OnChanges {
  ...
  _formDialogTitle:string='表单';
  _GUID:string = CommonUtility.uuID(6);

  constructor(private route:ActivatedRoute,private configService:ConfigService,private _broadcast:Broadcaster) {
  ...
  initGrid(config,filterItem = null){
    ...
    if(config && config.viewCfg){
      let masterResolver = new MasterGridViewResolver(config.viewCfg);
